...
$$ z_i \sim \text{Poisson}(q_{all} b_i), \quad q_{all} \sim \text{gamma}(\alpha_{all}, \beta_{all})$$
- The alternative hypothesis for each candidate cluster $S \in \mathcal{S}$, where $\mathcal{S}$ is the space of all possible clusters
$$
\begin{cases}
...
- 50 simulated data sets for each set of parameters
  ...
  - Outbreak effect ($\Delta = q_{in} - q_{out}$): 0.15, 0.20, 0.25, 0.30, 1.0, and 3.0^[Reporting rate 0.1 only for $\Delta = 1.0$ and 3.0.]
...
$$ q_{all} \sim \text{gamma}(1.5, 1) \\
q_{out} \sim \text{gamma}(1.5, 1) \\
q_{in} \sim \text{gamma}(1.5, 1) 
$$
...
df <- tribble(
  ~Method,            ~log_BF,   ~RR,
  "Naive 1",           875698.5,   NA_real_,
  "Under-reported 1",     713.95,   3.06,
  "Under-reported 2",     292.78,   2.71
)
...
df %>%
  ...
  fmt_number(
    columns = log_BF,
    decimals = 1,      # show one decimal like the photo
    use_seps = TRUE    # adds thousands separators
  ) %>%
  fmt_number(
    columns = RR,
    decimals = 2
  ) %>%
  # Put a superscript footnote marker on the NA in RR for Naive 1
  text_transform(
    locations = cells_body(rows = Method == "Naive 1", columns = RR),
    fn = function(x) ifelse(is.na(x), html("NA<sup>1</sup>"), x)
  ) %>%
  tab_footnote(
    footnote = md("Not provided in output from `ScanStatistic` package."),
    locations = cells_body(rows = Method == "Naive 1", columns = RR)
  ) %>%
  tab_options(table.font.names = "Helvetica", data_row.padding = px(6))
...
::: {.columns}
::: {.column}
![Null samples from simulation](images/mcmc_null_sim.png)
:::
::: {.column}
![Alternative samples from simulation](images/mcmc_alt_sim.png)
:::
::: 
...
::: {.columns}
::: {.column}
![Null samples from simulation](images/mcmc_null_tx.png)
:::
::: {.column}
![Alternative samples from simulation](images/mcmc_alt_tx.png)
:::
::: 
...
```{R}
p <- ggplot() +
  geom_function(fun = dbeta, args = list(shape1 = 3.5, shape2 = 23), aes(color = "0.1")) +
  geom_function(fun = dbeta, args = list(shape1 = 4.5, shape2 = 15), aes(color = "0.2")) +
  geom_function(fun = dbeta, args = list(shape1 = 10, shape2 = 22), aes(color = "0.3")) +
  geom_function(fun = dbeta, args = list(shape1 = 13, shape2 = 19), aes(color = "0.4")) +
  geom_function(fun = dbeta, args = list(shape1 = 2, shape2 = 2), aes(color = "0.5")) +
  xlim(c(0, 1)) +
  labs(title = "Prior Distribution on p", color = "True p")

q <- ggplot() +
  geom_function(fun = dgamma, args = list(shape = 2, scale = 0.5), aes(color = "q in and q all")) +
  geom_function(fun = dgamma, args = list(shape = 2, scale = 0.4), aes(color = "q out")) +
  geom_function(fun = dgamma, args = list(shape = 1.5, scale = 1), aes(color = "q COVID")) +
  xlim(c(0, 5)) +
  labs(title = "Prior Distribution on q", color = "Parameter")
```

## Appendix: Equations {.smaller}

$$
P(D|H_0) \propto \frac{\beta_{all}^{\alpha_{all}}}{\Gamma(\alpha_{all}) \mathcal{B}(\alpha, \beta)} \int \int q_{all}^{\alpha_{all} + C - 1} p^{\alpha + C - 1} (1 - p)^{\beta - 1} e^{-q_{all}\beta_{all} - p q_{all} B} dp dq_{all}
$$
$$
P(D|H_1(S)) \propto \frac{\beta_{in}^{\alpha_{in}}}{\Gamma(\alpha_{in}) \mathcal{B}(\alpha, \beta)} \int \int q_{in}^{\alpha_{in} + C_S - 1} p^{\alpha + C_S - 1} (1 - p)^{\beta - 1} e^{-q_{in}\beta_{in} - p q_{in} B_S} dp dq_{in} \times \\
\quad \frac{\beta_{out}^{\alpha_{out}}}{\Gamma(\alpha_{out}) \mathcal{B}(\alpha, \beta)} \int \int q_{out}^{\alpha_{out} + C_{S-G} - 1} p^{\alpha + C_{S-G} - 1} (1 - p)^{\beta - 1} e^{-q_{out}\beta_{out} - p q_{out} B_{S-G}} dp dq_{out}
$$
