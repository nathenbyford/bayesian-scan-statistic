...
- The marginal likelihood under a candidate region $i$ is now:
$P(D|H_1(S)) = \int \int \int P(D|q_{in}, q_{out}, p) \times \\ \quad \pi(q_{in}) \times \pi(q_{out}) \times \pi(p) dq_{in} dq_{out} dp$
...
cases <- counties |>
  ggplot() +
  geom_sf(aes(fill = `Probable Cases`)) +
  theme_void() +
  theme(
...
per_cap <- counties |> 
  ggplot() +
  geom_sf(aes(fill = `Probable Cases` / Population)) +
...
::::{.columns}
:::{.column width=30%}
![](images/confirmed_cases.png){width='60%' fig-align='right'}
:::
:::{.column width=30%}
![](images/population.png){width='60%' fig-align='center'}
:::
:::{.column width=30%}
![](images/texas_cases.png){width='70%' fig-align='left'}
:::
::::
